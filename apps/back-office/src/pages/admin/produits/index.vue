...
const filteredProducts = computed(() => {
  ...
  return adminStore.state.products.filter((product) => {
    if (
    ...
    if (
      query &&
      !`${product.name} ${product.ref} ${product.origin}`
        .toLowerCase()
        .includes(query)
    ) {
      return false;
...
function openProduct(productId: string) {
  router.push({ name: "admin-produits-modifier", params: { id: productId } });
}

function createProduct() {
  router.push({ name: "admin-produits-nouveau" });
}

function categoryLabel(categoryId: string) {
  return (
    adminStore.state.categories.find((category) => category.id === categoryId)
      ?.label || "—"
  );
}
</script>
...
<template>
  <div class="space-y-8 pb-10">
    ...
    >
      ...
      >
        ...
        <input
          v-model="search"
          type="search"
          placeholder="Produit, référence, origine…"
      ...
      <div
        class="ml-auto font-body text-[11px] uppercase tracking-[0.16em] text-cocoa/55"
      >
        {{ filteredProducts.length }} résultat<span
    ...
    <section class="overflow-hidden border border-cocoa/12 bg-ivory">
      <div
        class="grid grid-cols-[96px_minmax(0,2.2fr)_minmax(0,1.4fr)_minmax(0,1.2fr)_108px_92px_110px_84px] border-b border-cocoa/12 px-6 py-4 font-body text-[10px] uppercase tracking-[0.22em] text-cocoa/45"
      >
        <span>Réf.</span>
        <span>Nom</span>
        <span>Catégorie</span>
      ...
      <button
        ...
        :key="product.id"
        type="button"
        class="grid w-full grid-cols-[96px_minmax(0,2.2fr)_minmax(0,1.4fr)_minmax(0,1.2fr)_108px_92px_110px_84px] items-center gap-4 border-b border-cocoa/8 px-6 py-5 text-left transition-colors duration-200 hover:bg-beige/50 last:border-b-0"
      ...
      >
        ...
        <span class="min-w-0">
          <span class="block truncate font-display text-xl text-cocoa">{{
            product.name
          }}</span>
        ...
        <span class="font-body text-sm text-cocoa">{{
