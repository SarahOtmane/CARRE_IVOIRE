...
<template>
  <div class="space-y-10 pb-10">
    ...
    <!-- Paramètres généraux -->
    <form
      class="grid gap-8 border border-cocoa bg-ivory p-8 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.6fr)]"
      @submit.prevent="save"
    >
      ...
      <aside class="space-y-4">
        <div class="border border-cocoa p-5">
    ...
    <section class="space-y-6">
      <div class="flex items-end justify-between border-b border-cocoa pb-6">
        <div>
          <div class="font-body text-[10px] uppercase tracking-[0.28em] text-cocoa/45">Fiscalité</div>
          <h3 class="mt-3 font-display text-3xl text-cocoa">
            Taux de TVA
          </h3>
          <p class="mt-2 font-body text-sm text-cocoa/55">
      ...
      <div v-if="showTvaForm" class="border border-cocoa bg-ivory p-6">
        <div class="font-body text-[10px] uppercase tracking-[0.22em] text-cocoa/45 mb-5">
          {{ editingTvaId !== null ? "Modifier le taux" : "Nouveau taux de TVA" }}
        </div>
        <div class="grid gap-5 sm:grid-cols-[minmax(0,1fr)_180px]">
          <label class="grid gap-2">
            <span class="font-body text-[10px] uppercase tracking-[0.22em] text-cocoa/55">Libellé</span>
            <input
          ...
          <label class="grid gap-2">
            <span class="font-body text-[10px] uppercase tracking-[0.22em] text-cocoa/55">Taux (%)</span>
            <input
        ...
        <div class="mt-5 flex gap-3">
          <button
            type="button"
            :disabled="tvaLoading"
            class="border border-cocoa bg-cocoa px-5 py-3 font-body text-[11px] uppercase tracking-[0.16em] text-ivory disabled:opacity-50"
            @click="saveTva"
          >
          ...
          <button
            type="button"
            class="border border-cocoa px-5 py-3 font-body text-[11px] uppercase tracking-[0.16em] text-cocoa"
            @click="cancelTvaForm"
          >
            Annuler
          </button>
      ...
      <!-- Liste des TVA -->
      <div v-if="tvaLoading && taxRates.length === 0" class="py-10 text-center font-body text-sm italic text-cocoa/45">
        Chargement…
      </div>

      <div v-else-if="taxRates.length === 0 && !showTvaForm" class="border border-cocoa bg-ivory px-8 py-10 text-center">
        <p class="font-body text-sm italic text-cocoa/45">Aucun taux de TVA configuré.</p>
        <button
          type="button"
          class="mt-4 border border-cocoa px-5 py-3 font-body text-[11px] uppercase tracking-[0.16em] text-cocoa"
          @click="openCreateTva"
        >
      ...
      <div v-else-if="taxRates.length > 0" class="overflow-hidden border border-cocoa bg-ivory">
        <div class="grid grid-cols-[minmax(0,1fr)_120px_80px] border-b border-cocoa px-6 py-4 font-body text-[10px] uppercase tracking-[0.22em] text-cocoa/45">
          <span>Libellé</span>
        ...
        <div
          ...
          :key="tva.id"
          class="grid grid-cols-[minmax(0,1fr)_120px_80px] items-center gap-4 border-b border-cocoa px-6 py-5 last:border-b-0"
        >
          <span class="font-display text-xl text-cocoa">{{ tva.label }}</span>
          <span class="text-right font-body text-sm tabular-nums text-gold">{{ tva.rate }} %</span>
          <div class="flex justify-end gap-3">
            <button
              type="button"
              class="font-body text-[11px] uppercase tracking-[0.18em] text-cocoa/55 hover:text-cocoa hover:underline"
