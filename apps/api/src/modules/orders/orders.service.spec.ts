...
describe('OrdersService', () => {
  ...
  let productModel: { update: jest.Mock }
    ...
    const module = await Test.createTestingModule({
      providers: [
        ...
        },
        {
          ...
          useValue: { update: jest.fn().mockResolvedValue([1]) },
        },
        {
          provide: getModelToken(ProductVariant),
          useValue: { update: jest.fn().mockResolvedValue([1]), findByPk: jest.fn() },
        },
        {
  ...
  describe('createOrder', () => {
    ...
    it('lève OUT_OF_STOCK si rowsAffected === 0', async () => {
      productModel.update.mockResolvedValue([0])
